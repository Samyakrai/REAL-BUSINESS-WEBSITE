    ...
    def do_POST(self):
        if self.path == '/submit-feedback':
            ...
            try:
                ...
                self._set_headers()
                response = {'status': 'success', 'message': 'Feedback saved to database.'}
                self.wfile.write(json.dumps(response).encode('utf-8'))
                print(f"Feedback saved for {email}")
...
def run(server_class=HTTPServer, handler_class=FeedbackHandler, port=PORT):
    init_db()
    server_address = ('', port)
